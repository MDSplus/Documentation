...
* `!` at the beginning of a line denies access. Examples:
  * `! username@*` block the user `username` from anywhere
...
#### Read-only
This maps all users to the `nobody` account, effectively denying access.
...
* the second line accepts connections from anywhere and maps to them to the `nobody` user 
