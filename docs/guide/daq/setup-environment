...
> TODO: include info for Windows

There are two `setup` scripts in the MDSplus installation directory: `setup.sh` for **bash** and **zsh**, `setup.csh` for **tcsh**. One of these scripts needs to be [`source`](https://www.linuxcommand.org/lc3_man_pages/sourceh.html)ed in order to use MDSplus. 

The setup script will configure system environment variables such as `$PATH` and MDSplus-specific ones such as `$MDS_PATH`, both in the current process and for all subprocesses. Below are some ways to configure this so that it happens automatically. 

* To configure it for an entire system, you can symlink the MDSplus setup script so that it is run for all login shells with this command `ln -s /usr/local/mdsplus/setup.sh /etc/profile.d/mdsplus.sh`. This is done automatically by the MDSplus installer [TODO: Stephen to confirm]. 
...
The setup files work by parsing `envsyms` (see below) and other configuration files. Those files are written in a custom custom configuration file language similar to shell script.

When configuring an experiment, you may also want to add your own environment variables, which can be done once you understand how the configuration file language works. To learn more about the default variable set, see the next section on the `envsyms` file [below](#envsyms); for a full list of environment variables that affect MDSplus, see the [reference page](environment-variables.md).

* Comment lines begin with `#`
* Empty lines are ignored
* Any white space (spaces, tabs, etc.) is fine.
    * For example: `name value` will set `$name=value`.
    * The value can be wrapped in double quotes.
    * To clear a variable, set it to `""`
    * (TODO: Stephen to find if there's a way to unset aka delete a variable altogether)
* `name`/`value` can contain environment variables.
    
    example: `MDS_PATH	$MDSPLUS_DIR/tdi` sets the variable `MDS_PATH` to `$MDSPLUS_DIR/tdi`
...
It is highly recommended not to modify this file. Instead, you may save customizations as separate files in one of the following:
* `$MDSPLUS_DIR/local/envsyms`
* `/etc/mdsplus.conf`
* `$HOME/.mdsplus`
...
* It is recommended to set variables before modifying (append/prepend) them if possible. This is not always possible, e.g., `$PATH`, which is set by your system. It is possible and common for `setup.sh` to be run multiple times for various reasons. This will prevent environment variables from getting duplicates appended to it. 
    ...
    * `dir` can be `<` to prepend or `>` to append. `sep` is usually either a colon or semicolon
    
        > (TODO: Stephen will double-check)
