...
* For `alpha`, grab the desired version from [https://mdsplus.org/dist/Windows/alpha/](https://mdsplus.org/dist/Windows/alpha/?C=N;O=D).
* For `stable`, grab the desired version from [https://mdsplus.org/dist/Windows/stable/](https://mdsplus.org/dist/Windows/stable/?C=N;O=D).

When downloading or running the installer, several security warnings may appear. We are working to verify our installer to remove these warnings.
...
## Java Applications
MDSplus for Windows no longer includes the Java Run-time Environment in the installation kit. To run the Java applications (jScope and Traverser) provided with the MDSplus installation, you will need to also install either a Java Run-time Environment (JRE) or the Java Development Kit (jdk). These kits can be found on at [http://www.oracle.com](http://www.oracle.com). Be sure to install either a JRE or a JDK kit found in "Downloads for developers". The download on the Oracle site entitled "Java for your computer" provides only the browser support for Java and is not sufficient to run the Java applications provided with MDSplus.