...
Puts data into an MDSplus tree node. This routine uses the java or python interface, which you must choose with `mdsUsePython` (see also: `mdsUsePython`).
...
## `mdstcl(/COMMAND)`
Run TCL command or open a TCL prompt. This function provides Matlab with the same MDSTCL interface as IDL. (See also: [MDSTCL](../../tools/mdsdcl/mdstcl.md))
